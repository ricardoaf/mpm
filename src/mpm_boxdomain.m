...
n_cell = permute(n_cell', [2 1 3]);
node_matrix = reshape(1:node.n, n_cell+1);
idx_p = 1:elem.n_node;
for m = 1:dim-1, idx_p([4*m-1 4*m]) = idx_p([4*m 4*m-1]); end
for e = 1:elem.n
    [i,j,k] = ind2sub(n_cell, e);
    idx_i = [i i+1]; idx_i(idx_i>n_cell(1)+1) = [];
    idx_j = [j j+1]; idx_j(idx_j>n_cell(2)+1) = [];
    idx_k = [k k+1]; idx_k(idx_k>n_cell(3)+1) = [];
...
[R{2},R{1},R{3}]=meshgrid(R{2},R{1},R{3});
r_coord=[R{1}(:) R{2}(:) R{3}(:)];
...
for e = 1:elem.n
    ...
    active_elem = true;
    for i = 1:dim
        if any(e_coord(:,i)<0), active_elem = false; break; end
        if any(e_coord(:,i)>len(i)), active_elem = false; break; end
    end
    if active_elem
        np = size(e_coord,1);
...
part.body_force(:,2) = -density*abs(gravity_acc);
